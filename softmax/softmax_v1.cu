...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./softmax_v1 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N] [--extreme]\n";
...
__device__ float block_reduce_max(float value, float* warp_results) {
  ...
  if (warp == 0) {
    float block_value = lane < warp_count ? warp_results[lane] : -FLT_MAX;
    block_value = warp_reduce_max(block_value);
    if (lane == 0) {
      warp_results[0] = block_value;
    }
  }
  __syncthreads();
...
__device__ float block_reduce_sum(float value, float* warp_results) {
  ...
  if (warp == 0) {
    float block_value = lane < warp_count ? warp_results[lane] : 0.0f;
...
__global__ void softmax_shuffle_kernel(const float* input, float* output,
                                       int cols) {
...
void launch_softmax(const float* input, float* output, int rows, int cols,
  ...
  softmax_shuffle_kernel<<<rows, threads, warp_count * sizeof(float)>>>(
      input, output, cols);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v1_shuffle rows=" << options.rows
