...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./softmax_v4 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N] [--extreme]\n";
...
__global__ void softmax_block_cached_float4_kernel(const float* input,
  ...
  float4 value = active ? row_input[lane] : make_float4(-FLT_MAX, -FLT_MAX,
                                                        -FLT_MAX, -FLT_MAX);
...
const char* select_path(int cols) {
  if (cols <= 128) return "warp_cached";
  if (cols <= 1024 && (cols & 3) == 0) return "block_cached_float4";
...
void launch_softmax(const float* input, float* output, int rows, int cols) {
  ...
  if (cols <= 128) {
    ...
    softmax_warp_cached_kernel<<<blocks, threads>>>(input, output, rows, cols);
  } else if (cols <= 1024 && (cols & 3) == 0) {
    softmax_block_cached_float4_kernel<<<rows, threads,
                                         (threads / 32) * sizeof(float)>>>(
        input, output, cols);
  } else {
    softmax_block_general_kernel<<<rows, threads,
                                   (threads / 32) * sizeof(float)>>>(input,
                                                                     output,
                                                                     cols);
  }
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v4_register_cache rows=" << options.rows
            << " cols=" << options.cols << " path=" << select_path(options.cols)
