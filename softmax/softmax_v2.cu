...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./softmax_v2 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N] [--extreme]\n";
...
__device__ float block_reduce_max(float value, float* warp_results) {
  ...
  __syncthreads();
  if (warp == 0) {
    float block_value = lane < (blockDim.x >> 5) ? warp_results[lane] : -FLT_MAX;
    block_value = warp_reduce_max(block_value);
    if (lane == 0) warp_results[0] = block_value;
  }
  __syncthreads();
...
__device__ float block_reduce_sum(float value, float* warp_results) {
  ...
  __syncthreads();
  if (warp == 0) {
    float block_value = lane < (blockDim.x >> 5) ? warp_results[lane] : 0.0f;
...
__global__ void softmax_float4_kernel(const float* input, float* output,
                                      int cols) {
  ...
  if (aligned) {
    ...
    for (int i = lane; i < vector_count; i += blockDim.x) {
      const float4 value = row_input[i];
      thread_max = fmaxf(thread_max, value.x);
      thread_max = fmaxf(thread_max, value.y);
      thread_max = fmaxf(thread_max, value.z);
      thread_max = fmaxf(thread_max, value.w);
    }
  } else {
  ...
  const float row_max = block_reduce_max(thread_max, warp_results);
  ...
  if (aligned) {
    ...
    for (int i = lane; i < vector_count; i += blockDim.x) {
      ...
      const float4 value = {expf(source.x - row_max), expf(source.y - row_max),
                            expf(source.z - row_max), expf(source.w - row_max)};
      row_output[i] = value;
  ...
  const float inverse_sum =
      1.0f / block_reduce_sum(thread_sum, warp_results);
...
void launch_softmax(const float* input, float* output, int rows, int cols,
                    int threads) {
  softmax_float4_kernel<<<rows, threads, (threads / 32) * sizeof(float)>>>(
      input, output, cols);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v2_float4 rows=" << options.rows
            << " cols=" << options.cols << " threads=" << threads
            << " vectorized=" << ((options.cols & 3) == 0 ? 1 : 0)
