...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./transpose_v4 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N]\n";
...
__global__ void transpose_float4_kernel(const float* input, float* output,
                                        int rows, int cols) {
  __shared__ float tile[kTile][kTile + 1];

  // blockDim=(8,32): every thread owns four adjacent columns of one input row.
  const int input_row = blockIdx.y * kTile + threadIdx.y;
  const int input_vector_col = blockIdx.x * (kTile / 4) + threadIdx.x;
  const float4 value =
      reinterpret_cast<const float4*>(input)[
          static_cast<size_t>(input_row) * (cols / 4) + input_vector_col];
  const int tile_col = threadIdx.x * 4;
  tile[threadIdx.y][tile_col] = value.x;
  tile[threadIdx.y][tile_col + 1] = value.y;
  tile[threadIdx.y][tile_col + 2] = value.z;
  tile[threadIdx.y][tile_col + 3] = value.w;
  __syncthreads();

  // Swap tile coordinates. Four adjacent original rows become one float4 in an
  // output row. Padding keeps the four shared reads free of structural conflict.
  const int output_row = blockIdx.x * kTile + threadIdx.y;
  const int output_vector_col = blockIdx.y * (kTile / 4) + threadIdx.x;
  const int tile_row = threadIdx.x * 4;
  const float4 transposed =
      make_float4(tile[tile_row][threadIdx.y],
                  tile[tile_row + 1][threadIdx.y],
                  tile[tile_row + 2][threadIdx.y],
                  tile[tile_row + 3][threadIdx.y]);
  reinterpret_cast<float4*>(output)[
      static_cast<size_t>(output_row) * (rows / 4) + output_vector_col] =
      transposed;
}

__global__ void transpose_scalar_fallback_kernel(const float* input,
...
bool uses_float4(int rows, int cols) {
  return (rows % kTile == 0) && (cols % kTile == 0);
}

void launch_transpose(const float* input, float* output, int rows, int cols) {
  const dim3 grid((cols + kTile - 1) / kTile,
                  (rows + kTile - 1) / kTile);
  if (uses_float4(rows, cols)) {
    const dim3 block(kTile / 4, kTile);
    transpose_float4_kernel<<<grid, block>>>(input, output, rows, cols);
  } else {
    const dim3 block(kTile, kBlockRows);
    transpose_scalar_fallback_kernel<<<grid, block>>>(input, output, rows, cols);
  }
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v4_vectorized rows=" << options.rows
            << " cols=" << options.cols
            << " path=" << (uses_float4(options.rows, options.cols)
                                  ? "float4_8x32"
                                  : "scalar_32x8_fallback")
            << " tile=32x33\n"
