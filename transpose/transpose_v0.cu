...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./transpose_v0 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N]\n";
...
__global__ void transpose_direct_kernel(const float* input, float* output,
                                        int rows, int cols) {
...
void launch_transpose(const float* input, float* output, int rows, int cols) {
  ...
  transpose_direct_kernel<<<grid, block>>>(input, output, rows, cols);
...
int main(int argc, char** argv) {
  ...
  // Jetson reports the locked GPU clock through memoryClockRate on this device,
  // so it cannot be used as the LPDDR5 clock. The project benchmark environment
  // locks a 128-bit LPDDR5 interface at 3199 MHz: 2 * 3.199 GHz * 16 B ~= 102.4.
  constexpr double peak_memory_gbps = 102.4;
  ...
  const double peak_percent = 100.0 * effective_gbps / peak_memory_gbps;

  std::cout << std::fixed << std::setprecision(6)
            << "device=" << properties.name << " sm_count="
            << properties.multiProcessorCount << "\n"
            << "version=v0_direct rows=" << options.rows
            << " cols=" << options.cols << " block=32x8\n"
            << "max_abs_error=" << max_absolute_error
            << " max_rel_error=" << max_relative_error << "\n"
            << "median_ms=" << median_ms
            << " effective_bandwidth_GBps=" << effective_gbps
            << " peak_bandwidth_GBps=" << peak_memory_gbps
