...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./transpose_v2 [--rows=N] [--cols=N] [--warmup=N] "
                   "[--repeat=N]\n";
...
void launch_transpose(const float* input, float* output, int rows, int cols) {
  ...
  const dim3 grid((cols + kTile - 1) / kTile,
                  (rows + kTile - 1) / kTile);
  transpose_padded_kernel<<<grid, block>>>(input, output, rows, cols);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v2_padded rows=" << options.rows
            << " cols=" << options.cols << " tile=32x33 block=32x8 padding=1\n"
