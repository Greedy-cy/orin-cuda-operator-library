...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./gemm_v1 [--m=N] [--n=N] [--k=N] [--warmup=N] "
                   "[--repeat=N]\n";
...
__global__ void sgemm_tiled_kernel(const float* a, const float* b, float* c,
                                   int m, int n, int k) {
  ...
  for (int tile_begin = 0; tile_begin < k; tile_begin += kTile) {
    const int a_col = tile_begin + tx;
    const int b_row = tile_begin + ty;
    tile_a[ty][tx] =
        (row < m && a_col < k) ? a[static_cast<size_t>(row) * k + a_col]
                               : 0.0f;
    tile_b[ty][tx] =
        (b_row < k && col < n) ? b[static_cast<size_t>(b_row) * n + col]
                               : 0.0f;
    __syncthreads();

#pragma unroll
    for (int inner = 0; inner < kTile; ++inner) {
      sum = fmaf(tile_a[ty][inner], tile_b[inner][tx], sum);
    }
    __syncthreads();
  }
...
void launch_custom(const float* a, const float* b, float* c, int m, int n,
  ...
  sgemm_tiled_kernel<<<grid, block>>>(a, b, c, m, n, k);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v1_shared_tile m=" << options.m << " n=" << options.n
            << " k=" << options.k << " tile=" << kTile << "\n"
