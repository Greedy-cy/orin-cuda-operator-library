...
inline Options parse_options(int argc, char** argv, const char* binary_name) {
  Options options;
...
inline void launch_cublas(cublasHandle_t handle, const float* a,
...
template <typename Launch>
int run_benchmark(int argc, char** argv, const char* binary_name,
                  const char* version, const char* configuration,
                  Launch launch_custom) {
  const Options options = parse_options(argc, argv, binary_name);
