...
__global__ __launch_bounds__(kThreads) void sgemm_cp_async_kernel(
  ...
  cp_async_16(&tile_a[0][a_row][a_col],
              &a[static_cast<size_t>(block_row + a_row) * k + a_col]);
  cp_async_16(&tile_b[0][b_row][b_col],
  ...
  for (int tile_index = 0; tile_index < tile_count; ++tile_index) {
    ...
    if (has_next) {
      ...
      cp_async_16(
          &tile_a[write_buffer][a_row][a_col],
          &a[static_cast<size_t>(block_row + a_row) * k + next_begin + a_col]);
      cp_async_16(
          &tile_b[write_buffer][b_row][b_col],
...
void launch_custom(const float* a, const float* b, float* c, int m, int n,
  ...
  if (m % kBlockM == 0 && n % kBlockN == 0 && k % kBlockK == 0) {
    sgemm_cp_async_kernel<<<grid, block>>>(a, b, c, m, n, k);
  } else {
