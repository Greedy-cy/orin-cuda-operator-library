...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./rmsnorm_v2 [--tokens=N] [--hidden=N] "
...
__global__ void rmsnorm_fused_shuffle_kernel(const float* input,
...
void launch_rmsnorm(const float* input, const float* weight, float* output,
  ...
  rmsnorm_fused_shuffle_kernel<<<tokens, threads,
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v2_fused_shuffle tokens=" << options.tokens
