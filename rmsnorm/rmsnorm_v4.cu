...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (parse_int("--tokens=", &options.tokens) ||
        ...
        parse_int("--threads=", &options.threads) ||
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./rmsnorm_v4 [--tokens=N] [--hidden=N] "
                   "[--threads=0|128|256|512] [--epsilon=F] [--warmup=N] "
                   "[--repeat=N]\n";
  ...
  const bool valid_threads = options.threads == 0 || options.threads == 128 ||
                             options.threads == 256 || options.threads == 512;
  if (options.tokens <= 0 || options.hidden <= 0 || options.warmup < 0 ||
      options.repeat <= 0 || options.epsilon <= 0.0f || !valid_threads) {
    std::cerr << "tokens, hidden, repeat and epsilon must be positive; warmup "
                 "must be non-negative; threads must be 0, 128, 256 or 512\n";
    std::exit(EXIT_FAILURE);
  }
  return options;
}
...
__global__ void rmsnorm_float4_config_kernel(
    const float4* input, const float4* weight, float4* output,
    int vectors_per_row, int hidden, float epsilon) {
  extern __shared__ float warp_results[];
  const int token = blockIdx.x;
  const size_t row_offset = static_cast<size_t>(token) * vectors_per_row;
  float square_sum = 0.0f;
  for (int index = threadIdx.x; index < vectors_per_row; index += Threads) {
...
template <int Threads>
__global__ void rmsnorm_scalar_config_kernel(
  ...
  float square_sum = 0.0f;
  for (int col = threadIdx.x; col < hidden; col += Threads) {
...
int select_threads(int hidden, int requested) {
  if (requested != 0) return requested;
  // The Orin scan selected 256 for both supported hidden sizes. 128 does not
  // improve hidden=1024, while 512 regresses the 32-token hidden=4096 case.
  (void)hidden;
  return 256;
}

template <int Threads>
void launch_config(const float* input, const float* weight, float* output,
                   int tokens, int hidden, float epsilon) {
  constexpr int shared_bytes = (Threads / 32) * sizeof(float);
  if ((hidden & 3) == 0) {
    rmsnorm_float4_config_kernel<Threads><<<tokens, Threads, shared_bytes>>>(
    ...
    rmsnorm_scalar_config_kernel<Threads><<<tokens, Threads, shared_bytes>>>(
        input, weight, output, hidden, epsilon);
  }
}

void launch_rmsnorm(const float* input, const float* weight, float* output,
                    int tokens, int hidden, float epsilon, int threads) {
  switch (threads) {
    case 128:
      launch_config<128>(input, weight, output, tokens, hidden, epsilon);
      break;
    case 256:
      launch_config<256>(input, weight, output, tokens, hidden, epsilon);
      break;
    case 512:
      launch_config<512>(input, weight, output, tokens, hidden, epsilon);
      break;
  }
...
int main(int argc, char** argv) {
  ...
  const int threads = select_threads(options.hidden, options.threads);
  ...
  for (int i = 0; i < options.warmup; ++i)
    launch_rmsnorm(device_input, device_weight, device_output, options.tokens,
                   options.hidden, options.epsilon, threads);
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v4_config_dispatch tokens=" << options.tokens
            << " hidden=" << options.hidden << " threads=" << threads
