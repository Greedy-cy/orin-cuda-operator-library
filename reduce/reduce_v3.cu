...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./reduce_v3 [--n=N] [--warmup=N] [--repeat=N]\n";
...
__global__ void reduce_ilp4_kernel(const float* input, float* output, size_t n) {
  extern __shared__ float shared[];
  const unsigned int lane = threadIdx.x;
  const size_t index = blockIdx.x * blockDim.x + lane;
  const size_t stride = static_cast<size_t>(blockDim.x) * gridDim.x;
  const size_t loop_stride = stride * 4;

  // Four independent accumulation chains let the scheduler overlap multiple
  // outstanding global loads instead of waiting on one load-add dependency.
  float sum0 = 0.0f;
  float sum1 = 0.0f;
  float sum2 = 0.0f;
  float sum3 = 0.0f;
...
void launch_reduce(const float* input, float* output, size_t n, int blocks,
                   int threads) {
  reduce_ilp4_kernel<<<blocks, threads, threads * sizeof(float)>>>(input, output,
                                                                  n);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v3_ilp4 n=" << options.n << " blocks=" << blocks
            << " threads=" << threads << "\n"
