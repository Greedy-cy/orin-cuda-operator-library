...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./reduce_v4 [--n=N] [--warmup=N] [--repeat=N]\n";
...
__global__ void reduce_float4_kernel(const float* input, float* output,
                                     size_t n) {
  extern __shared__ float shared[];
  const unsigned int lane = threadIdx.x;
  const size_t index = blockIdx.x * blockDim.x + lane;
  const size_t stride = static_cast<size_t>(blockDim.x) * gridDim.x;
  const size_t vector_count = n / 4;

  const float4* vector_input = reinterpret_cast<const float4*>(input);
  float sum0 = 0.0f;
  float sum1 = 0.0f;
  float sum2 = 0.0f;
  float sum3 = 0.0f;

  for (size_t i = index; i < vector_count; i += stride) {
  ...
  // cudaMalloc guarantees sufficient alignment for float4. The scalar tail
  // keeps arbitrary N correct without weakening the aligned fast path.
  for (size_t i = vector_count * 4 + index; i < n; i += stride) {
    sum0 += input[i];
  }
...
void launch_reduce(const float* input, float* output, size_t n, int blocks,
                   int threads) {
  reduce_float4_kernel<<<blocks, threads, threads * sizeof(float)>>>(input,
                                                                     output, n);
...
int main(int argc, char** argv) {
  ...
  const size_t vector_work = options.n / 4;
  const size_t scalar_tail = options.n % 4;
  const size_t work_items = std::max<size_t>(vector_work, scalar_tail);
  const size_t required_blocks = (work_items + threads - 1) / threads;
  const int blocks = static_cast<int>(
      std::min<size_t>(std::max<size_t>(required_blocks, 1),
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v4_float4 n=" << options.n << " blocks=" << blocks
            << " threads=" << threads << "\n"
