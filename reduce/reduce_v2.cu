...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./reduce_v2 [--n=N] [--warmup=N] [--repeat=N]\n";
...
__global__ void reduce_shuffle_kernel(const float* input, float* output,
                                      size_t n) {
  ...
  // Shared-memory reduction stops at one warp. Compared with v1 this removes
  // the final shared-memory stages and their block-wide barriers.
...
void launch_reduce(const float* input, float* output, size_t n, int blocks,
                   int threads) {
  reduce_shuffle_kernel<<<blocks, threads, threads * sizeof(float)>>>(input,
                                                                      output, n);
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v2_shuffle n=" << options.n << " blocks=" << blocks
            << " threads=" << threads << "\n"
