...
Options parse_options(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help") {
      std::cout << "Usage: ./reduce_v0 [--n=N] [--warmup=N] [--repeat=N]\n";
...
__global__ void reduce_atomic_kernel(const float* input, float* output,
                                     size_t n) {
  ...
  // This is intentionally the baseline: every participating thread performs
  // one atomic operation on the same global address.
  if (index < n) {
...
int main(int argc, char** argv) {
  ...
  std::cout << std::fixed << std::setprecision(6)
            ...
            << "version=v0_atomic n=" << options.n << " blocks=" << blocks
            << " threads=" << threads << "\n"
